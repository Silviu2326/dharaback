...
const checkAvailability = asyncHandler(async (req, res, next) => {
  const { therapistId, date_time, duration, location_id, include_reasons, exclude_booking_id } = req.query;
  ...
  // Exclude the current booking if provided (for rescheduling)
  if (exclude_booking_id) {
    query = query.neq('id', exclude_booking_id);
  }

  const { data: existingBookings, error } = await query.limit(1);
